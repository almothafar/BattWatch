...
- **`translatable="false"` is not protection.** It keeps a string out of the translation files and does nothing about formatting, since the digits follow the device locale rather than the file the string came from. An untranslated `%1$d` renders `٤٠` on an Arabic device exactly like a translated one — `battery_level_percent` did, until #273.
- **No string resource may contain `%d` at all.** Every number is formatted in code, so a numeric placeholder in a resource has no legitimate use; `StringResourceDigitsTest` fails the build on one in any `values*` file.
